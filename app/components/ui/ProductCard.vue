...
<template>
  <div
    class="group relative flex flex-col bg-surface-light dark:bg-dark-card-alt rounded-2xl overflow-hidden
           border border-border-light dark:border-white/5
           hover:border-primary/30 dark:hover:border-primary/30
           hover:shadow-lg transition-all duration-300"
  >
    <!-- Image -->
    <NuxtLink :to="localePath(`/shop/${product.slug}`)" class="block relative overflow-hidden aspect-[4/5]">
      <img
        :src="product.image"
      ...
      <button
        class="absolute bottom-3 left-3 right-3 py-2.5 bg-background-dark/90 hover:bg-primary text-white hover:text-background-dark
    ...
    <div class="p-4 flex flex-col gap-1">
      <NuxtLink :to="localePath(`/shop/${product.slug}`)">
        <h3 class="font-semibold text-text-main dark:text-white leading-snug hover:text-primary transition-colors">
          {{ product.name }}
        </h3>
        <p class="text-xs text-text-muted dark:text-white/50 mt-0.5">{{ product.nameJa }}</p>
      </NuxtLink>

      <div class="flex items-center justify-between mt-2">
        <span class="font-bold text-text-main dark:text-white">{{ formattedPrice }}</span>
        <span v-if="product.shortDescription" class="text-xs text-text-muted dark:text-white/50 truncate ml-2">
